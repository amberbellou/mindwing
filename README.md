...
- **Between levels**: one focused lesson card per concept, each ending in a practical "Field note" the player can use the same day. A light runs around the card's frame while a ten-second reading time counts down, and the card cannot be skipped before it ends; the hint says so, and the frame holds gold when the quick check opens. Meanwhile a neon underline sweeps through the text word by word at about 2.8 words a second, a comfortable read-aloud pace, brightening each word as it passes, so young readers have a place to rest their eyes.

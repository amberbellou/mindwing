...
| 5 The Engine's Roost | Offloading and drift | The boss's fireballs split into faded decoys with no bright centre. They cannot burn you, but touching one shoves you off your line. In its final phase the Engine pauses to reload; only then do sparks land in full, so timing beats spamming. |

Two smaller devices run through all levels. A fleeting golden "glimmer" appears for two seconds and pays if caught; the first one is named on screen as a variable reward, so the pattern is easier to spot in an app. And the player's clarity move changes meaning halfway through: on levels 1 and 2 it clears the sky, on levels 3 to 5 it destroys nothing and instead freezes everything for a moment while you choose your own line, which is the retention and drift lesson played out in the controls.
